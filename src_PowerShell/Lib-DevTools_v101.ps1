...
# ------------------------------------------------------------------------------
# --- [ルートA] 物理座標ベースの要素逆引きおよび座標補正 (最強の特定精度) ---
function Invoke-BrowserPointReverse {
    ...
    if ($null -ne $global:WebViewCtrl) {
        try {
            $pt = New-Object System.Drawing.Point($ScreenPoint.X, $ScreenPoint.Y)
            $clientPt = $global:WebViewCtrl.PointToClient($pt)
            $clientX = $clientPt.X
            $clientY = $clientPt.Y
        } catch {
    ...
    # 2. ブラウザ内でのヒットテスト
    <# 論理座標(X, Y)からフロントエンドのDOM要素を elementsFromPointによるZ軸スタック取得を用い、
       透過レイヤー、多層Shadow DOM、および同一オリジンのiframeを貫通して、インタラクティブ要素(input, button等)を抽出 #>
    $jsPoint = @"
    try {
        let targetX = $clientX;
        let targetY = $clientY;
        ...
        function deepElementFromPoint(x, y, rootNode) {
            ...
            function probeShadow(root, px, py) {
                try {
                    ...
                    const candidates = root.querySelectorAll ? root.querySelectorAll('input,button,textarea,select,[role="search"],[role="textbox"]') : [];
            ...
            for (let i = 0; i < stacked.length; i++) {
                ...
                try {
                    const inner = el.querySelector && el.querySelector('input,textarea,select,button,[role="textbox"],[role="search"]');
                    ...
                    if (el.shadowRoot) {
                        ...
                        try {
                            ...
                            if (inner && inner !== el) {
                                ...
                                const innerInput = inner.querySelector && inner.querySelector('input,textarea,select,button,[role="textbox"],[role="search"]');
...
"@
    ...
    # 3. 【座標補正ロジック】
    <# JSの elementFromPoint で特定した要素を、CDP側のノード群と突合させ、
       ズレの無い CDP の真の BoundingBox で上書き（補正）する #>
    if ($domData.boundingBox -and $global:DomNodes) {
        $matchedNode = $null

        # セレクタまたはタグ名/ID/Nameによる特定
