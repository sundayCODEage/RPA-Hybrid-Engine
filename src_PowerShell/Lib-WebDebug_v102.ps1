...
function Convert-DomSnapshotFlat {
    ...
    # ----------------------------------------------------------
    # [1] DOM構造の展開
    <# CDPから返却された並列配列（同じインデックス番号 $i に各プロパティが格納されている）を
       ループ処理で巡回し、バラバラになった属性情報やタグ名を1つの連想配列（$node）に組み立て直す。 #>
    ...
    if ($layout -and $layout.nodeIndex -and $layout.styles) {
        ...
        for ($L = 0; $L -lt $layout.nodeIndex.Count; $L++) {
            ...
            # list内の該当ノードを取得
            if ($nodeIdx -ge 0 -and $nodeIdx -lt $list.Count) {
                $targetNode = $list[$nodeIdx]
                ...
                for ($s = 0; $s -lt $styleValues.Count -and $s -lt $StyleNames.Count; $s++) {
                    ...
                    if ($strIdx -ge 0 -and $strIdx -lt $strings.Count) {
                        $styleName = $StyleNames[$s]
                        $styleValue = $strings[$strIdx]
                        
                        # 抽出したスタイルをノードの computedStyles へ格納
                        $targetNode.computedStyles[$styleName] = $styleValue
                    }
                }
            }
        }
